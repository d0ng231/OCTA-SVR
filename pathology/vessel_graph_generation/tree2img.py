...
def _parse_node_str(s: str) -> tuple[float, ...]:
    """Robustly parse a node position serialized as a string.

    Accepts formats like "[x y z]", "(x, y, z)", "[x, y]", etc.
    Returns a tuple of floats with length 2 or 3.
    """
    s = s.strip()
    # Trim surrounding brackets/parentheses if present
    if s and s[0] in "([" and s[-1] in ")]":
        s = s[1:-1]
    # Normalize delimiters to whitespace and split
    for ch in ",[]()":
        s = s.replace(ch, " ")
    toks = [t for t in s.split() if len(t) > 0]
    vals: list[float] = []
    for t in toks:
        try:
            vals.append(float(t))
...
def rasterize_forest(forest: list[dict],
    ...
    for edge in forest:
        ...
        # Skip only if below minimum; clamp large radii instead of dropping
        ...
        if isinstance(edge["node1"], np.ndarray) or isinstance(edge["node1"], list):
            ...
            # Robust parser for serialized tuples/lists with spaces or commas
            current_node = _parse_node_str(edge["node1"])  # type: ignore[arg-type]
            proximal_node = _parse_node_str(edge["node2"])  # type: ignore[arg-type]
...
def voxelize_forest(forest: dict,
    ...
    for edge in forest:
        ...
        if isinstance(edge["node1"], np.ndarray) or isinstance(edge["node1"], list):
            ...
            # Legacy and robust: accept commas and brackets for both endpoints
            current_node = _parse_node_str(edge["node1"])  # type: ignore[arg-type]
            proximal_node = _parse_node_str(edge["node2"])  # type: ignore[arg-type]
