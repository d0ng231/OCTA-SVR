...
class Greenhouse():
    ...
    def __init__(self, config: dict):
        ...
        # Optional cap of rotation influence vs FAZ radius (keeps original default when absent)
        try:
            self.rotation_radius_factor_vs_faz = float(config.get('rotation_radius_factor_vs_faz', 0.0) or 0.0)
        except Exception:
            self.rotation_radius_factor_vs_faz = 0.0
        # Optional ramp-up iterations for rotation influence (to avoid early FAZ-edge voids when jittered)
        try:
            self.rotation_radius_ramp_iters = int(config.get('rotation_radius_ramp_iters', 30) or 30)
        except Exception:
            self.rotation_radius_ramp_iters = 30
        # Add optional FAZ center randomness (uniform disk or Gaussian), then clamp
        _faz_center = np.array(config.get('FAZ_center', [0.5, 0.5]), dtype=float)
        jitter_rad = float(config.get('FAZ_center_jitter', 0.0) or 0.0)
        jitter_std = config.get('FAZ_center_jitter_std', None)
        self._faz_jitter_active = False
        if jitter_rad and jitter_rad > 0.0:
            ang = np.random.uniform(0.0, 2.0 * np.pi)
            r_u = np.sqrt(np.random.uniform(0.0, 1.0)) * jitter_rad
            _faz_center = _faz_center + r_u * np.array([np.cos(ang), np.sin(ang)], dtype=float)
            self._faz_jitter_active = True
        elif jitter_std is not None:
            try:
                if isinstance(jitter_std, (list, tuple)) and len(jitter_std) == 2:
                    jx, jy = float(jitter_std[0]), float(jitter_std[1])
                else:
                    jx = jy = float(jitter_std)
                _faz_center = _faz_center + np.array([np.random.normal(0.0, jx), np.random.normal(0.0, jy)], dtype=float)
                self._faz_jitter_active = True
            except Exception:
                pass
        _faz_center[0] = float(np.clip(_faz_center[0], 0.05, 0.95))
        _faz_center[1] = float(np.clip(_faz_center[1], 0.05, 0.95))
        self.FAZ_center: Tuple[float, float] = (float(_faz_center[0]), float(_faz_center[1]))
    ...
    def develop_forest(self):
        ...
        for mode in self.modes:
            ...
            for t in range(t,t+self.I):
                ...
                # pbar updates removed for non-interactive use
            ...
            if node.is_leaf:
                ...
                if np.std(angles) > self.phi and (self.FAZ_radius==0 or ((dist_to_center / (2*self.FAZ_radius))**5 > random.uniform(0,1) and get_angle_between_two_vectors(vector_to_center, avg_attraction_vector[:2])>90)):
                    ...
                    if self.rotation_radius>0 and t>15:
                        # Original rotation guidance around FAZ, with optional effective cap vs FAZ size
                        g = norm_vector(g)
                        ...
                        rot_eff = float(self.rotation_radius)
                        try:
                            cap = float(getattr(self, 'rotation_radius_factor_vs_faz', 0.0) or 0.0)
                            if cap > 0.0:
                                rot_eff = min(rot_eff, float(self.FAZ_radius) * cap)
                        except Exception:
                            pass
                        # Ramp rotation influence during early iterations when FAZ jitter is active to avoid inner voids
                        weight = max(0.01 if not first_mode else 0, rot_eff - dist_new_pos_to_center)
                        weight = sqrt(weight)
                        if getattr(self, '_faz_jitter_active', False):
                            ramp = 1.0
                            try:
                                ramp = float(min(1.0, max(0.0, t / max(1, self.rotation_radius_ramp_iters))))
                            except Exception:
                                ramp = 1.0
                            weight = weight * ramp
                        ...
                        # Slightly reduce outward component during ramp-up to prevent banding
                        out_coeff = 0.3
                        if getattr(self, '_faz_jitter_active', False):
                            try:
                                ramp = float(min(1.0, max(0.0, t / max(1, self.rotation_radius_ramp_iters))))
                                ...
                                ramp = 1.0
                            out_coeff = 0.1 + 0.2 * ramp
                        g = (1-weight)*g + 0.7*weight*ort_vector + out_coeff*weight*out_vector
    ...
    # (Reverted) No additional FAZ-border suppression helper; use original rules
