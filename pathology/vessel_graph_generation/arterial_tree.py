...
class Node(anytree.NodeMixin):
    ...
    def optimize_edge_radius_to_root(self):
        ...
        if not self.is_root and not self.is_leaf:
            # Exclude microaneurysm branches from Murray optimization so they
            # do not thicken upstream vessels.
            real_children = [c for c in self.children if not getattr(c, 'is_microaneurysm', False)]
            if len(real_children) == 0:
                # Only MA children present: skip updating this edge and do not propagate.
                return
            r_p = sum([child.radius ** self.kappa for child in real_children]) ** (1 / self.kappa)
            if self.radius == r_p:
                return
